...
class cnnrnnModel(object):
    def __init__(self,config,is_training,embeddingl,initializer):
        ...
        self.sentence_embeddings_expanded=tf.expand_dims(self.embedding_words,-1)
        with tf.name_scope("cnn"):
            filterc=tf.get_variable("filter3",[self.filter_size,self.wordEmbeddingSize,1,self.num_filters],initializer=initializer)
            conv=tf.nn.conv2d(self.sentence_embeddings_expanded,filterc,strides=[1,1,1,1],padding="VALID",name="conv")
            b=tf.get_variable("bias",[self.num_filters],initializer=initializer)
            h=tf.nn.relu(tf.nn.bias_add(conv,b),name="relu")#[batchsize,seq-filtr+1,1,num_filters]
            self.cnnem=tf.reshape(h,[-1,self.wordsnum-self.filter_size+1,self.num_filters])
        
        #binput2=tf.split(self.cnnem,self.wordsnum-self.filter_size+1,1)  #list,words,batch,1,emb       
        ...
        with tf.name_scope("lstm"):
            ...
            bhidden_state_1,_=tf.nn.dynamic_rnn(bcell,self.cnnem,dtype=tf.float32)
            '''
            batch=tf.shape(self.xforward)[0]            
            self.binitial_state=bcell.zero_state(batch,tf.float32)  #initial state  t=0 :c and h                   
            self.boutput=[]
        ...
        #self.final_docp=bhidden_state_1[:,self.wordsnum-1-self.filter_size+1,:]
        #final mlp
