...
class cnnrnnattSing(object):
    def __init__(self,config,is_training,embeddingl,initial):
        ...
        with tf.name_scope("cnn"):
            filterc=tf.get_variable("filter3",[self.filter_size,self.wordEmbeddingSize,1,self.num_filters],initializer=initial)
            conv=tf.nn.conv2d(self.sentence_embeddings_expanded,filterc,strides=[1,1,1,1],padding="VALID",name="conv")
            b=tf.get_variable("bias",[self.num_filters],initializer=initial)
        ...
        sh_r1=tf.nn.tanh(sa)
        sh_r=tf.reshape(sh_r1,[-1,self.wordsnum-self.filter_size+1,self.wordtotext_hidsize])#batch,sens,hid
